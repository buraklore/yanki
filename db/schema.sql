...
-- ---------- v2: measurement depth (2026-09) ---------------------------
-- Every statement here is idempotent, in keeping with the file's contract.

-- §10 — explicit ranking vs textual order. 'explicit' means the judge read a
-- numbered/ordered recommendation list in the answer; 'textual' means rank is
-- first-appearance order. The two must never be conflated in reporting.
alter table answer_runs add column if not exists rank_kind text;
-- Judge prompt/model version per row: changing the judge changes comparability,
-- so the row must say which judge produced it.
alter table answer_runs add column if not exists judge_version text;

-- §15 — page-level citations. One representative URL per (run, domain); the
-- unique (run_id, domain) constraint is kept so the migration cannot conflict
-- with existing rows, which carry domain only and stay valid.
alter table run_citations add column if not exists url text;

-- §12 — brands the judge saw in answers that we do not track yet. The judge
-- reports them in a separate field that can never affect scoring; they exist
-- only so the operator can promote or dismiss them.
create table if not exists candidate_brands (
  id           uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references workspaces(id) on delete cascade,
  name         text not null,           -- as the model wrote it
  norm_key     text not null,           -- entity.key(name), dedupe key
  mentions     integer not null default 1,
  first_seen   timestamptz not null default now(),
  last_seen    timestamptz not null default now(),
  sample_prompt_id uuid references prompts(id) on delete set null,
  status       text not null default 'pending',   -- pending | added | dismissed
  unique (workspace_id, norm_key)
);
create index if not exists candidate_brands_ws_idx
  on candidate_brands (workspace_id, status, mentions desc);

-- §21/§26 — persisted action items. Cards are generated deterministically in
-- the Action Center; `ref` is the stable identity of a card, so marking one
-- done survives re-renders and re-scans, and done_at dates become the
-- before/after markers on the trend chart.
create table if not exists recommendations (
  id           uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references workspaces(id) on delete cascade,
  ref          text not null,           -- stable card id, e.g. audit:llms_txt
  kind         text not null,           -- opportunity|audit|platform|source|perception
  title        text not null,
...
create index if not exists recommendations_ws_idx
  on recommendations (workspace_id, status, created_at desc);
